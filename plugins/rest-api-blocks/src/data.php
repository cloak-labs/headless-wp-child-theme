  ...
  if ($block_object && $block_object->block_type) {
    ...
    if ($attributes) {
      foreach ($attributes as $key => $attribute) {
        ...
        // ****** START Kaelan's customizations
        // special modifications to plugin by Kaelan that makes REST API responses include more ACF field information and better data formatting for select field types (relationships, repeaters, images).
        if ($key == 'data') {
          $data = $attr[$key];
          $modifiedData = [];
          ...
          foreach ($data as $key => $value) { // loop through fields in 'data' (ACF fields)
            if (str_starts_with($key, '_') && str_starts_with($value, 'field_')) { // pick out the fields that have the ACF field names 
              ...
              // When false is returned if no field object found, set as empty array to suppress notices
              if (!$acf_field_object) {
                $acf_field_object = [];
              }
              ...
              // by default, ACF relationship fields return an array of post IDs. Here we modify it to return the related posts' full data so we can eliminate the need to make multiple separate requests:
              if ($acf_field_object['type'] == 'relationship' || $acf_field_object['type'] == 'page_link') {
                if ($field_value) {
                  $related_posts = [];
              ...
              if ($acf_field_object['type'] == 'image') {
              ...
              if ($acf_field_object['type'] == 'repeater') {
                $repeaterFieldsFound[] = $acf_field_object;
              }

              $field_types_to_return_full_acf_object = ['repeater', 'relationship', 'page_link'];

              if (in_array($acf_field_object['type'], $field_types_to_return_full_acf_object)) {
          ...
          if ($repeaterFieldsFound) {
            foreach ($repeaterFieldsFound as $repeater) { // loop through repeaters (each one being a full ACF field object)
              ...
              $count = 0;
              while ($count < $repeater['value']) { // loop through repeater's blocks/children
                $repeater_block = [];

                foreach ($repeater['sub_fields'] as $sub_field) { // loop through repeater sub fields
                  $sub_field_name = $sub_field['name'];
                  $sub_field_api_default_name = $repeater_name . '_' . $count . '_' . $sub_field_name; // this string is the default key for the current sub_field
                  $sub_field_object = $modifiedData[$sub_field_api_default_name];
                  $repeater_block[$sub_field_name] = $sub_field_object;
                  unset($modifiedData[$sub_field_api_default_name]); // remove sub_field from top-level, as we're nesting it within its parent value
                }

                $final_repeater_value[] = $repeater_block;
                $count++;
              }
              $modifiedData[$repeater_name]['value'] = $final_repeater_value; // finally, replace repeater's value with the formatted array of repeater blocks
            }
          }

          $attr['data'] = $modifiedData; // finally, set the data object which will be returned in API responses
        }
        // ****** END Kaelan's customizations
